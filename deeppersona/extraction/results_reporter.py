...
class ResultsReporter:
    ...
    def report_from_agents(self,
                          agents: Union[DeepPersona, DeepWorld, List[DeepPersona]],
        ...
        Args:
            agents: Single agent, DeepWorld, or list of agents to interview.
    ...
    def report_from_interactions(self,
        ...
        Args:
            agents: Single agent, DeepWorld, or list of agents to analyze.
            report_title: Title for the generated report.
    ...
    def _extract_agents(self, agents) -> List[DeepPersona]:
        """Extract a list of DeepPersona objects from various input types."""
        if isinstance(agents, DeepPersona):
            return [agents]
        elif isinstance(agents, DeepWorld):
            return agents.agents
        elif isinstance(agents, list):
            return agents
        else:
            raise ValueError("Agents must be a DeepPersona, DeepWorld, or list of DeepPersona objects.")
    
    def _interview_agent(self, agent: DeepPersona, reporting_task: str) -> str:
        """Interview a single agent about the reporting task."""
        if self.verbose:
            logger.debug(f"Interviewing agent {agent.name} about: {reporting_task}")
        
        # Following DeepPersona patterns - directly interact with the agent
        prompt = f"""
