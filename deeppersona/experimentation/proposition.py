...
from deeppersona.utils import LLMChat, indent_at_current_level
from deeppersona.experimentation import logger
...
class Proposition:
    ...
    def __init__(self, claim:str, target=None, include_personas:bool=False, first_n:int=None, last_n:int=None,
        ...
        """ 
        Define a proposition as a (textual) claim about a target, which can be a DeepWorld, a DeepPersona or several of any.
        ...
        Args:
            ...
            target (DeepWorld, DeepPersona, list): the target or targets of the proposition. If not given, it will have to be specified later.
...
#
    ...
    def _build_context(self, current_targets):
        ...
        for target in current_targets:
            ...
            if isinstance(target, DeepPersona):
            ...
            elif isinstance(target, DeepWorld):
                if self.include_personas:
    ...
    def _target_as_list(self, target):
        if target is None:
            return None 
        elif isinstance(target, DeepWorld) or isinstance(target, DeepPersona):
            return [target]
        elif isinstance(target, list) and all(isinstance(t, DeepWorld) or isinstance(t, DeepPersona) for t in target):
            return target
        else:
            raise ValueError("Target must be a DeepWorld, a DeepPersona or a list of them.")
...
def check_proposition(target, claim:str, additional_context="No additional context available.",
    ...
    Args:
        target (DeepWorld, DeepPersona, list): the target or targets of the proposition
        claim (str): the claim of the proposition
