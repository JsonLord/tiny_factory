...
class Simulation:
    ...
    def __init__(self, id="default", cached_trace:list=None):
        ...
        self.factories = [] # e.g., DeepPersonaFactory instances
        ...
        self.cache_path = f"./deeppersona-{id}.cache.json" # default cache path
    ...
    def begin(self, cache_path:str=None, auto_checkpoint:bool=False):
        ...
        from deeppersona.environment import DeepWorld
        from deeppersona.factory.deep_persona_factory_base import DeepPersonaFactoryBase as DeepPersonaFactory
        ...
        DeepWorld.clear_environments()
        DeepPersonaFactory.clear_factories()
        DeepPersonaFactory.clear_factories()
        ...
        for agent_state in state["agents"]:
            try:
                ...
                if agent.environment is None:
                    if DeepPersona.communication_display:
    ...
    def __init__(self, obj_under_transaction, simulation, function, *args, **kwargs):
        ...
        if simulation is not None:
            if hasattr(obj_under_transaction, 'simulation_id') and obj_under_transaction.simulation_id is not None:
                ...
                # if is a DeepPersona, add the agent to the simulation
                if isinstance(obj_under_transaction, DeepPersona):
                    simulation.add_agent(obj_under_transaction)
                    logger.debug(f">>>>>>>>>>>>>>>>>>>>>>> Added agent {obj_under_transaction} to simulation {simulation.id}.")

                # if is a DeepWorld, add the environment to the simulation
                elif isinstance(obj_under_transaction, DeepWorld):
                    simulation.add_environment(obj_under_transaction)
                
                # if is a DeepPersonaFactory, add the factory to the simulation
                elif isinstance(obj_under_transaction, DeepPersonaFactory):
                    ...
                    raise ValueError(f"Object {obj_under_transaction} (type = {type(obj_under_transaction)}) is not a DeepPersona or DeepWorld instance, and cannot be captured by the simulation.")
        ...
        if self.simulation is None or self.simulation.status == Simulation.STATUS_STOPPED:
            ...
            if self.simulation._is_transaction_event_cached(event_hash, 
                ...
                if not self.simulation.is_under_parallel_transactions():
                    ...
                    # Output encoding/decoding is used to preserve references to DeepPersona and DeepWorld instances
