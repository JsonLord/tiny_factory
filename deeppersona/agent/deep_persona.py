from deeppersona.agent import logger, default, Self, AgentOrWorld, CognitiveActionModel
from deeppersona.agent.memory import EpisodicMemory, SemanticMemory, EpisodicConsolidator
from deeppersona.agent.social_types import ConnectionEdge, BehavioralEvent, InfluenceProfile, Content, Reaction
import deeppersona.openai_utils as openai_utils
from deeppersona.utils import JsonSerializableRegistry, repeat_on_error, name_or_empty
import deeppersona.utils as utils
from deeppersona.control import transactional, current_simulation
from deeppersona import config_manager
from deeppersona.utils.logger import get_logger
...
import threading
from deeppersona.utils import LLMChat  # Import LLMChat from the appropriate module

import deeppersona.utils.llm
...
#######################################################################################################################
@utils.post_init
class DeepPersona(JsonSerializableRegistry):
    """A simulated person in the DeepPersona universe."""
    ...
    def __init__(self, name:str=None, 
        ...
        """
        Creates a DeepPersona.

        Args:
            name (str): The name of the DeepPersona. Either this or spec_path must be specified.
        ...
        assert name is not None, "A DeepPersona must have a name."
        self.name = name
    ...
    def _post_init(self, **kwargs):
        ...
        from deeppersona.agent.action_generator import ActionGenerator # import here to avoid circular import issues
        ...
        if not hasattr(self, '_mental_faculties'):
            # This default value MUST NOT be in the method signature, otherwise it will be shared across all instances.
            from deeppersona.agent.mental_faculty import SequentialThinkingFaculty
            self._mental_faculties = [SequentialThinkingFaculty()]
        
        if self.enable_browser:
            from deeppersona.agent.browser_faculty import BrowserFaculty
        ...
        self._prompt_template_path = os.path.join(
            os.path.dirname(__file__), "prompts/deep_persona.mustache"
        )
    ...
    def get(self, key):
        """
        Returns the value of a key in the DeepPersona's persona configuration.
    ...
    def include_persona_definitions(self, additional_definitions: dict):
        """
        Imports a set of definitions into the DeepPersona. They will be merged with the current configuration.
    ...
    def define(self, key, value, merge=False, overwrite_scalars=True):
        """
        Define a value to the DeepPersona's persona configuration. Value can either be a scalar or a dictionary.
    ...
    def define_relationships(self, relationships, replace=True):
        """
        Defines or updates the DeepPersona's relationships.
    ...
    def clear_relationships(self):
        """
        Clears the DeepPersona's relationships.
        """
    ...
    def related_to(self, other_agent, description, symmetric_description=None):
        ...
        Args:
            other_agent (DeepPersona): The other agent.
        ...
        Returns:
            DeepPersona: The agent itself, to facilitate chaining.
        """
    ...
    def act(
        ...
        if n is not None:
            assert n < DeepPersona.MAX_ACTIONS_BEFORE_DONE
        ...
        def aux_act_once():
            ...
            if self.enable_basic_action_repetition_prevention and \
               (DeepPersona.MAX_ACTION_SIMILARITY is not None) and (next_action_similarity > DeepPersona.MAX_ACTION_SIMILARITY):
            ...
            if utils.first_non_none(communication_display, DeepPersona.communication_display):
        ...
        ##### Option 2: run until DONE ######
            ...
            while (len(contents) == 0) or (
                ...
                if len(contents) > DeepPersona.MAX_ACTIONS_BEFORE_DONE:
...
max_content_length=max_content_length,
    ...
    def _update_cognitive_state(
        ...
        """
        Update the DeepPersona's cognitive state.
        """
    ...
    def __repr__(self):
        return f"DeepPersona(name='{self.name}')"

    @transactional()
    def minibio(self, extended=True, requirements=None):
        """
        Returns a mini-biography of the DeepPersona.
        ...
        for stimus in content["stimuli"]:
            if simplified:
                ...
                msg_simplified_content = textwrap.fill(
                    msg_simplified_content,
                    width=DeepPersona.PP_TEXT_WIDTH,
    ...
    def load_specification(path_or_dict, suppress_mental_faculties=False, suppress_memory=False, suppress_mental_state=False, 
        ...
        return DeepPersona.from_json(json_dict_or_path=path_or_dict, suppress=suppress_attributes,
                                    serialization_type_field_name="type",
    ...
    def load_specifications_from_folder(folder_path:str, file_suffix=".agent.json", suppress_mental_faculties=False, 
        ...
        for file in os.listdir(folder_path):
            if file.endswith(file_suffix):
                ...
                agent = DeepPersona.load_specification(file_path, suppress_mental_faculties=suppress_mental_faculties,
    ...
    def encode_complete_state(self) -> dict:
        """
        Encodes the complete state of the DeepPersona, including the current messages, accessible agents, etc.
    ...
    def decode_complete_state(self, state: dict) -> Self:
        """
        Loads the complete state of the DeepPersona, including the current messages,
        and produces a new DeepPersona instance.
        """
        state = copy.deepcopy(state)
        
        self._accessible_agents = [DeepPersona.get_agent_by_name(name) for name in state["_accessible_agents"]]
    ...
    def create_new_agent_from_current_spec(self, new_name:str) -> Self:
        ...
        """
        new_agent = DeepPersona(name=new_name, spec_path=None)
    ...
    def add_agent(agent):
        ...
        """
        if agent.name in DeepPersona.all_agents:
            raise ValueError(f"Agent name {agent.name} is already in use.")
        else:
            DeepPersona.all_agents[agent.name] = agent
    ...
    def has_agent(agent_name: str):
        ...
        """
        return agent_name in DeepPersona.all_agents
    ...
    def set_simulation_for_free_agents(simulation):
        ...
        if desired.
        """
        for agent in DeepPersona.all_agents.values():
    ...
    def get_agent_by_name(name):
        ...
        """
        if name in DeepPersona.all_agents:
            return DeepPersona.all_agents[name]
        else:
            return None
    ...
    def all_agents_names():
        ...
        """
        return list(DeepPersona.all_agents.keys())
    ...
    def predict_reaction(self, content: Content) -> Reaction:
        ...
        """
        from deeppersona.llm_predictor import LLMPredictor
