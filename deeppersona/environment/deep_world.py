from deeppersona.environment import logger, default
...
from deeppersona.agent import *
from deeppersona.utils import name_or_empty, pretty_datetime
import deeppersona.control as control
from deeppersona.control import transactional
from deeppersona import utils
from deeppersona import config_manager
 
from rich.console import Console

from typing import Any, TypeVar, Union
AgentOrWorld = Union["DeepPersona", "DeepWorld"]
...
class DeepWorld:
    ...
    def __init__(self, name: str=None, agents=[], 
        ...
        if name is not None:
            ...
            self.name = f"DeepWorld {utils.fresh_id(self.__class__.__name__)}"
        ...
        DeepWorld.add_environment(self)
    ...
    def _step(self, 
        ...
        for intervention in self._interventions:
            ...
            if should_apply_intervention:
                if DeepWorld.communication_display:
    ...
    def add_agent(self, agent: DeepPersona):
        """
        Adds an agent to the environment. The agent must have a unique name within the environment.

        Args:
            agent (DeepPersona): The agent to add to the environment.
    ...
    def remove_agent(self, agent: DeepPersona):
        """
        Removes an agent from the environment.

        Args:
            agent (DeepPersona): The agent to remove from the environment.
        """
    ...
    def get_agent_by_name(self, name: str) -> DeepPersona:
        """
        ...
        Returns:
            DeepPersona: The agent with the specified name.
        """
    ...
    #######################################################################
    @transactional()
    def _handle_actions(self, source: DeepPersona, actions: list):
        """ 
        Handles the actions issued by the agents.

        Args:
            source (DeepPersona): The agent that issued the actions.
    ...
    @transactional()
    def _handle_reach_out(self, source_agent: DeepPersona, content: str, target: str):
        """
        ...
        Args:
            source_agent (DeepPersona): The agent that issued the REACH_OUT action.
    ...
    @transactional()
    def _handle_talk(self, source_agent: DeepPersona, content: str, target: str):
        """
        Handles the TALK action by delivering the specified content to the specified target.

        Args:
            source_agent (DeepPersona): The agent that issued the TALK action.
            content (str): The content of the message.
    ...
    def __repr__(self):
        return f"DeepWorld(name='{self.name}')"
    ...
    def decode_complete_state(self, state:dict):
        ...
        for agent_state in state["agents"]:
            try:
                try:
                    agent = DeepPersona.get_agent_by_name(agent_state["name"])
    ...
    def add_environment(environment):
        ...
        """
        if environment.name in DeepWorld.all_environments:
            raise ValueError(f"Environment names must be unique, but '{environment.name}' is already defined.")
        else:
            DeepWorld.all_environments[environment.name] = environment
    ...
    def set_simulation_for_free_environments(simulation):
        ...
        if desired.
        """
        for environment in DeepWorld.all_environments.values():
    ...
    def get_environment_by_name(name: str):
        ...
        Returns:
            DeepWorld: The environment with the specified name.
        """
        if name in DeepWorld.all_environments:
            return DeepWorld.all_environments[name]
        else:
            return None
